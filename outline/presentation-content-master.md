...
### 본 문서 누적 영역 (v1.0 신설 차수)

- `outline/step-page-matrix.md` v10.5 §9-NEW 영역 (27장)
- `dist/preview-step1.html` v10 빌드 본문 (Step 1 9장)
- `dist/preview-step2.html` v3.1 빌드 본문 (Step 2 11장·12·16 + 09·10·11·13·14·15·17·18·19)
- `spec/content-thesis.md` (§1 사례 정체성·§2 보조 논지·§4 표현 금지·§5 변경 이력)
- `CLAUDE.md` v2.5 §13 가드 1~56
- 본 세션 누적 영역 (docx → md 정합 검증·1·2·3순위 통합 반영)

### 관련 문서

- `outline/step-page-matrix.md` v10.5 — 강제 명시 영역 (시각·디자인)
- `outline/slides.yml` v1.0 — 28장 골조 (Phase 2 산출물)
- `spec/content-thesis.md` — 사례 정체성·보조 논지
- `spec/design-tokens.md` v1.6·`component-catalog.md` v1.6 — 시각 어휘
- `CLAUDE.md` v2.5 — 가드 1~56·§13 세션 운영 가이드
- `SESSION_STATE.md` — 운영 메모·본인 환경 영역

### 갱신 규칙

- 본 문서는 발표 본문 영역만 누적. 매트릭스·spec·dist 빌드 영역 변경 X.
- 다음 차수 진입 시 본 문서 §10 변경 이력 갱신.
- 발표 직전 본 문서 최종 정합 검증 권장.

---

## §1. 슬라이드별 발표 본문 (현 빌드·매트릭스 §9-NEW 영역)

총 27장 — Step 1 8장 + Step 2 11장 + Step 3 7장 + Step 4·5 (미작성). 빌드 상태: ○ 빌드 완료·△ 매트릭스 반영·빌드 미반영·**X 미빌드**.
...
**빌드 상태**: ○ (preview-step1.html v10·수직 계열화 v9 적용 완료)
...
**본문 영역** (docx Step 1 §4 정합·6대 특성):
- **그룹 1 운영 중단 리스크** (설비 가동률):
  1. 자본집약적 장치산업 — 대규모 설비 투자·연속공정·가동률·다운타임 = 손익 직결
  2. 24시간 연속 가동 — 정지 = 막대한 손실·다운타임 통제 핵심
- **그룹 2 시장 변동 대응** (실시간 의사결정):
  3. 경기순환·원료가 변동성 — 유가·환율에 따른 마진 변동·실시간 수익성·재고 통제
  4. 산업의 쌀·전후방 연계 — 자동차·반도체·건설 등 모든 산업 영향
- **그룹 3 통합·규제 복잡성** (글로벌·M&A·규제):
  5. 글로벌 다거점 — 6국 생산·110국 수출·통화·세무 통합
  6. M&A 포트폴리오 확대 — 인수 이후 기준정보 충돌
  7. 규제·품질·안전관리 — EHS·화학물질·환경 규제·기록·추적성 (※ docx는 (6)으로 별도 추가)

**시각 어휘**: industry-pictogram 6개 + 그룹별 카드·하단 색바 액센트

**출처**: docx Step 1 §4·[11] 이상신 박사논문 (재인용 김근옥 2019)·[1] 박제성 인터뷰 (자본집약적 대규모 장치산업)

**빌드 상태**: ○ (preview-step1.html v10 — 빌드 페이지 07로 위치)
...
**빌드 상태**: ○ (preview-step1.html v10·v10 신설·빌드 페이지 9번째·hp-num "06")
...
- 3축 카드 (card-top-accent 4px red + num-circle 50px red + SVG 픽토그램 56px + 제목 38px + sub 20px + body 26px):
  1. **발주·의사결정** (롯데케미칼): 김교현 대표·박제성 상무 — 의사결정 주체 + 자원 배분 + 요구사항 정의. 그룹 IT 자산 통제권 유지 전략적 판단 — 출처 [1] [8]
...
### 20. 20 chapter3-intro — Step 3 챕터 진입 (v10.4 신규·미빌드)

**핵심 메시지**: "**24시간 가동되는 3사 시스템을 멈추지 않고** 7개월 안에 통합해야 했다"

**시각 구조**: 챕터 진입·헤드라인 + 4대 제약 미리보기 카드 2×2 + 질문 박스

**본문 영역**:
- 챕터 라벨: "Step 3 · 7개월의 구축" big-num style 강조
- 4대 제약 미리보기 카드 2×2 (각 픽토그램 56px·시계·플랜트·합쳐지는 원·지구):
  1. **Time**: 7개월 압축 일정
  2. **Business Continuity**: 24시간 연속 가동
  3. **Multi-Entity**: 3사 병존 운영
  4. **Global Multi-Site**: 6국 + 110국
- 질문 박스 (v3-msg·26px): "어떻게 **안전하게 실행**했는가?"

**출처**: [1] [8]·docx Step 3 §1·slides.yml chapter 3 정합

**빌드 상태**: **X (미빌드·매트릭스 §9-NEW v10.4만 작성·다음 차수 Step 3 빌드 진입)**

---

### 21. 21 four-constraints — Step 3 4대 제약 본문 (v10.4 신규·미빌드)

**핵심 메시지**: "네 가지 제약이 동시에 충돌한 **고난도 통합 프로젝트**"

**시각 구조**: 4대 제약 카드 2×2·각 card-top-accent 4px red + 픽토그램 4 + 결론 메시지 박스

**본문 영역** (각 카드 num-circle 50px red + 제목 38px + sub 20px + body 26px):
- 카드 1 **Time · 7개월 압축** (sub "2017.04 ~ 2017.11"): "10개 모듈·3사·6국·110국 통합을 7개월에 완수. 동종 사례 평균 12~18개월 대비 1.5배 압축 일정"
- 카드 2 **Business Continuity · 24시간 가동** (sub "1분 정지도 손실"): "석유화학 연속 공정 1년 365일 가동 — 시스템 1분 정지도 수십억 원 손실. 다운타임 제로 요구"
- 카드 3 **Multi-Entity · 3사 병존 운영** (sub "롯데케미칼·정밀화학·첨단소재"): "3사 기준정보·프로세스 병존 + 통합 압력. 각 사 본연 경쟁력 유지 + 단일 디지털 코어 동시 안착"
- 카드 4 **Global Multi-Site · 6국 + 110국** (sub "글로벌 거점·수출"): "여수·대산·울산 국내 + 중국·말레이시아·영국·우즈베키스탄·파키스탄·미국 6국 + 110국 수출 통합"

**결론 메시지 박스** (v3-msg·26px):
- "4대 제약 동시 충돌 → **SAP Value Assurance Service** 기반 사전 진단 + 반복 검증 전략"
- 출처: [1] [8]
...
### 22. 22 methodology-evolution — Step 3 방법론 진화 + 비대칭성·새 디지털 코어 + 1610 + 4대 방법론 + 압축 일정 4근거 (v10.4·v10.5 통합)
...
### 23. 23 not-standardize-all — Step 3 GAP 진입 + 챕터 1 §8 회수 + AS-IS/TO-BE 6항목 (v10.5 신규)
...
### 24. 24 gap-three-tracks — Step 3 GAP 3트랙 본문 (v10.4 신규·미빌드)
...
### 25. 25 sap-platform-hub — Step 3 SAP 중심 플랫폼 + 주변 6 시스템 (v10.4 신규·미빌드·content-thesis §2 보조 논지 4 매핑)
...
### 26. 26 downtime-kpi — Step 3 다운타임 KPI + SAP Value Assurance + 6대 리스크 매트릭스 + 다운타임 의미 4 + 테스트 5종 (v10.4·v10.5 통합)
...
### 27. 27 conclusion (Step 4·5 미작성 영역)
